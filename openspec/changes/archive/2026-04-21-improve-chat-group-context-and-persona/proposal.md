...
The bot's @mention/reply chat feature gives low-quality answers because the AI has no awareness of the group's recent conversation — it responds in a vacuum. The in-group persona also relies on fragile regex post-processing to inject Vietnamese slang, which produces unnatural results and is hard to maintain.

## What Changes

- `src/services/chat.ts`: `chatWithAI()` accepts an optional `groupContext?: string` parameter; when provided it is injected into the system prompt inside `<recent_chat>...</recent_chat>` XML tags. The `CHAT_SYSTEM_PROMPT` is rewritten for a more vivid, natural in-group persona with slang baked in directly. Random emoticon/filler appending in `postProcessResponse()` is removed; phrase replacements are kept as a safety net.
...
- `chat-persona-rewrite`: Rewrite the in-group chat persona system prompt with natural Vietnamese slang baked in and remove the fragile regex post-processing appender.
